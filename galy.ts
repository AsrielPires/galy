...
import { $, body, C, close, doc, icon, Icon } from "galhui";
import { call, extend, isA, isU, l, lazy, notF, t } from "inutil";
import { Alias, bind, getTag, L, ontag, orray, remove, replace, setTag } from "orray";
...
export class Ctx extends E<ICtx, { change: never }>{
  ...
  prots(key: str) {
    ...
    if (!t)
      throw notF(key, null, "prototypes");
    return t;
  }
  item(item: iitems) {
    switch (item.tp) {
...
class Stack implements Item, Render {
  ...
  render() {
    ...
    return bind(orray(list, v => isA(v) ? v : <[int, iitems]>[100 / l(list), v]), div(["_", "stack", o]), {
...
class Tab implements Item, Render {
  ...
  render() {
    ...
    return div("_ tab", [
      bind(list, div(C.head), {
        tag: (v, a) => v.cls(C.on, a),
        insert: box => box.$.head().cls(C.item)
          .add(close(e => { e.stopPropagation(); remove(list, box); }))
          .on('click', () => setTag(list, "on", box))
      }),
      call(div(C.body), b => {
        let cb = (v: iBox) => {
          b.attr("id", false).uncls().cls(C.body);
          if (v) {
            b.set(v.$.render(true));
            //v.$.focus?.(b);
          } else b.set(i.empty?.());
...
class Box implements Item, Render {
  ...
  render(headerless?: bool) {
    ...
    return headerless ? bd.cls(C.item) : div("_ tab", [
      this.head().cls("_ bar"),
      div(C.body, bd)
    ]);
  }
